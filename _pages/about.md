---
permalink: /
title: "Yue Wang"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

I am a Ph.D. student in Computer Science at [Hong Kong Baptist University](https://www.hkbu.edu.hk/), advised by [Prof. Xiaoqing Guo](https://guo-xiaoqing.github.io/) in the [UltraVision+ Lab](https://ultravisionlab.github.io/). My research focuses on artificial intelligence for medical imaging, with a particular interest in 3D ultrasound reconstruction, neural rendering, and Gaussian Splatting.

My current research focuses on 3D reconstruction from 2D ultrasound sequences. I am interested in using AI to build spatial-aware medical image representations. My long-term interest is to integrate medical image reconstruction, 3D representation learning, and human-AI interaction to support more efficient clinical workflows, such as helping clinicians understand scanned regions, assess spatial coverage, and improve the efficiency and reliability of clinical practice.

Previously, I worked on accelerated MRI reconstruction and diffusion-based medical image reconstruction at the Shenzhen Institute of Advanced Technology, Chinese Academy of Sciences. I received my B.Eng. degree in Biomedical Engineering from Shenzhen University.

## News

- **2026** — 🎉 *Guided MRI Reconstruction via Schrödinger Bridge* was published in **IEEE Transactions on Medical Imaging**.
- **2025** — 🚀 Started my Ph.D. journey in **Computer Science at Hong Kong Baptist University**.
- **2025** — 🎤 Our work on accelerated quantitative $T_1\rho$ mapping was presented as an **Oral at ISMRM 2025**.
- **2024** — 🎓 Graduated from **Shenzhen University** with a B.Eng. in Biomedical Engineering.
- **2024** — ✨ *High-Frequency Space Diffusion Models for Accelerated MRI* and *SPIRiT-Diffusion* were published in **IEEE Transactions on Medical Imaging**.

You can find my CV here: [Curriculum Vitae](/files/CV.pdf).

[Email](mailto:yue.wang0908@gmail.com) / [Google Scholar](https://scholar.google.com/citations?user=-pqgxXsAAAAJ&hl=zh-CN) / [GitHub](https://github.com/YuebyYue)
